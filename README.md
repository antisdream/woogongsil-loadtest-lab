...
우공실 서비스의 성능 검증을 별도로 관리하기 위한 부하 테스트 저장소다.

이 저장소는 서비스 코드 저장소인 [antisdream/woogongsil](https://github.com/antisdream/woogongsil)과 분리해 운영한다. 우공실 본 저장소는 애플리케이션 코드와 배포 문서를 중심으로 관리하고, 이 저장소는 k6, Prometheus, Grafana 기반의 성능 테스트 환경과 테스트 기록을 관리한다.
...
`.env`에서 Grafana 관리자 계정 값을 로컬 환경에 맞게 조정한다.
...
`WGS_COOKIE` 값은 인증이 필요한 API 테스트에만 사용한다. Cookie, session token, 관리자 계정 정보는 결과 파일이나 Git 저장소에 남기지 않는다.
...
| 4차 | 2026-06-20 15:43 | [2026-06-20-loadtest-04.md](./docs/2026-06-20-loadtest-04.md) | [Naver Blog](https://blog.naver.com/andisdream/224321809266) |

## 공개 관리 기준

다음 항목은 Git 추적 대상에서 제외한다.
...
회차별 문서는 원문 블로그와 로컬 결과를 바탕으로 공개 가능한 수준의 목적, 조건, 결과, 개선 방향만 정리한다.
